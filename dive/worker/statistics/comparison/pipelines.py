...
def run_comparison_from_spec(spec, project_id, conditionals=[]):
    ...
    can_run_numerical_comparison_dependent = len([ dv for dv in dependent_variables if dv['general_type'] == 'q' ]) >= 2 and len(independent_variables_names) == 0
    ...
    if can_run_anova:
        ...
        anova_boxplot_data = get_anova_boxplot_data(project_id, dataset_id, df, independent_variables_names, dependent_variables_names)
        pairwise_comparison_data = get_pairwise_comparison_data(df, independent_variables_names, dependent_variables_names, significance_cutoff=significance_cutoff)
        result.update({
            'anova': anova,
    ...
    if can_run_numerical_comparison:
        if can_run_numerical_comparison_independent:
            numerical_comparison_data = run_valid_numerical_comparison_tests(df, independent_variables_names, independence=True)
        if can_run_numerical_comparison_dependent:
            numerical_comparison_data = run_valid_numerical_comparison_tests(df, dependent_variables_names, independence=False)
